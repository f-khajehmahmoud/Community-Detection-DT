...
def Embedded_COFOGA():
    ...
    def Sorting_Payoff_Communities(List_Payoff, ar):
        order = []
        ar1 = []
    ...
    for i in range(len(graph.nodes)):
        ...
        ar = ar + ar1
    for i in range(len(graph.nodes)):
        P = Calculating_utility(ar[i], Mat_Weigh, graph, E)
        List_Payoff.append(P)
    ar, List_Payoff = Sorting_Payoff_Communities(List_Payoff, ar)
        ...
        while (i < len(ar)):
            ...
            while (j < len(ar)):
                ...
                while (k < len(ar[i])):
                    kk = 0
                    while (kk < len(ar[j])):
                        if ((ar[i][k], ar[j][kk]) in graph.edges or (ar[j][kk], ar[i][k]) in graph.edges):
                            C = ar[i] + ar[j]
