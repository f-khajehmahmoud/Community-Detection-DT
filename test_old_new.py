...
UseOldCalculateLink=True
UseOldCalculateDegree=True
    ...
    for i in range(len(Community)):
        ...
        while(j<len(Community)):
            if (Community[i],Community[j]) in graph.edges:
                ...
                N_edge_old = N_edge_old + graph.number_of_edges(Community[i],Community[j])
            j=j+1
    # print(f"N_edge1=> {N_edge}")
    # print(f"N_edge2=> {N_edge_old}")
    if(UseOldCalculateLink):
        return N_edge_old
    return N_edge
...
def Calculating_Degree(Community,Mat_weigh,graph,E,N_edge):
    ...
    for (node,degree) in graph.degree(Community):
        N_degree_old=N_degree_old+degree

    # print(f"N_degree1=> {N_degree}")
    # print(f"N_degree2=> {N_degree_old}")
    if(UseOldCalculateDegree): 
        return N_degree_old
    return N_degree
